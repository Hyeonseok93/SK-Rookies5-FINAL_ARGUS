"""CLI and orchestration entry point for the ARGUS 1-2 final report."""
...
def generate_report(
    ...
    *,
    ...
    data_path = output_dir / "report-data.json"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate the ARGUS 1-2 HTML/PDF final report")
    parser.add_argument("--report", type=Path, default=backend_root / "data/report/1-2/latest.yaml")
    parser.add_argument("--evidence", type=Path, default=backend_root / "data/report/1-2/evidence")
    parser.add_argument("--output", type=Path, default=backend_root / "data/report/1-2/final")
    parser.add_argument("--no-pdf", action="store_true")
    args = parser.parse_args()
    try:
        result = generate_report(args.report, args.evidence, args.output, include_pdf=not args.no_pdf)
    except Exception as exc:
        error = {"ok": False, "section_id": "1-2", "error": f"{type(exc).__name__}: {exc}"}
