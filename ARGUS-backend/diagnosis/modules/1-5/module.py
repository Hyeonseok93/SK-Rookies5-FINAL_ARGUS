"""Diagnosis module 1-5: 검증되지 않은 리다이렉트와 포워드."""
...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g15_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 1-5 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g15_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G15Module(DiagnosisModule):
    section_id = "1-5"
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g15_scan(ctx, self.module_dir)
