"""Diagnosis module 7-2: 파일 목록화 가능성."""
...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g72_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 7-2 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g72_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G72Module(DiagnosisModule):
    section_id = "7-2"
    title = "파일 목록화 가능성"
    chapter = 7
    implemented = True
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g72_scan(ctx, self.module_dir)
