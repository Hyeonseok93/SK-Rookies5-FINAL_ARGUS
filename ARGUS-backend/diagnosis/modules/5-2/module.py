"""Diagnosis module 5-2: 요청 및 응답 값 내 주요정보 포함여부 확인."""
...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g52_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 5-2 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g52_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G52Module(DiagnosisModule):
    section_id = "5-2"
    title = "요청 및 응답 값 내 주요정보 포함여부 확인"
    chapter = 5
    implemented = True
    engine = "httpx"
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g52_scan(ctx, self.module_dir)
