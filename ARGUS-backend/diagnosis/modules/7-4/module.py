...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g74_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 7-4 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g74_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G74Module(DiagnosisModule):
    section_id = "7-4"
    title = "취약한 보안설정"
    chapter = 7
    implemented = True
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g74_scan(ctx, self.module_dir)
