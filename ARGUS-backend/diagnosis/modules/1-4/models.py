...
세 가지 입력 소스(URL List / API List / Swagger)를 단일 ScanTarget 객체로 정규화하기 위한 공통 데이터 모델을 정의
"""
...
class ScanTarget:
    ...
    tags: list = field(default_factory=list)     # OpenAPI 작업 태그
    # OpenAPI x-roles/x-allowed-roles 선언이며, 역할 이름은 동적으로 유지
    allowed_roles: list = field(default_factory=list)
    source: InputSource = InputSource.URL_LIST
    raw: Optional[str] = None              # 원본 입력 (디버깅/로그용)
    content_type: str = ""                 # OpenAPI requestBody 미디어 타입
    ...
    def from_raw_url(raw_url: str) -> "ScanTarget":
        """
        URL List 입력 한 줄을 ScanTarget으로 변환
