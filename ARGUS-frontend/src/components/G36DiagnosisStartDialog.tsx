...
import { FileArchive, Gauge, Layers, Settings2, Stethoscope, X, ZapOff } from "lucide-react";
import { G36DiagnosisOptionsPanel } from "./G36DiagnosisOptionsPanel";
import {
  FULL_G36_OPTIONS,
  G36_PRESET_LABELS,
  MINIMAL_G36_OPTIONS,
  ...
  type G36DiagnosisPreset,
} from "../lib/g36DiagnosisOptions";

const TABS: { id: G36DiagnosisPreset; icon: typeof Gauge; desc: string }[] = [
  { id: "minimal", icon: ZapOff, desc: "wordlist만 · Base×목록" },
  { id: "full", icon: Layers, desc: "wordlist + api-tree 전체" },
  { id: "manual", icon: Settings2, desc: "옵션 직접 설정" },
];

function PresetOverview({ preset, options }: { preset: "minimal" | "full"; options: G36DiagnosisOptions }) {
  const isMinimal = preset === "minimal";
  const checks = isMinimal
    ? ["내장 backup/test wordlist만", "모든 Base × wordlist GET", "anonymous → authenticated 2-pass", "수 분 이내"]
    : ["wordlist + api-tree 파일형 path 전수", "2-2 leak 판정 연동", "anonymous + test-account", "시간 크게 증가 가능"];
  return (
    <div className={`rounded-xl border p-4 ${isMinimal ? "border-cyan-400/30 bg-gradient-to-br from-cyan-500/10 to-transparent" : "border-amber-400/30 bg-gradient-to-br from-amber-500/10 to-transparent"}`}>
      <div className="mb-3 flex items-center gap-2">
        <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${isMinimal ? "bg-cyan-500/20 text-cyan-300" : "bg-amber-500/20 text-amber-200"}`}>
          {isMinimal ? <Gauge className="h-4 w-4" /> : <FileArchive className="h-4 w-4" />}
        </span>
        <div>
          <p className="text-sm font-semibold text-white">{G36_PRESET_LABELS[preset]}</p>
          <p className="text-[10px] text-cyber-muted">{isMinimal ? "백업·테스트 파일 빠른 확인" : "inventory 파일 path 전수 probe"}</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-cyber-border/40 bg-cyber-bg/50 px-3 py-2">
        ...
        <p className="mt-1 font-mono text-[10px] leading-relaxed text-cyan-300/90">{g36OptionsSummary(options)}</p>
      </div>
    </div>
  );
}

export function G36DiagnosisStartDialog({ open, initialOptions, onClose, onStart }: { open: boolean; initialOptions: G36DiagnosisOptions; onClose: () => void; onStart: (options: G36DiagnosisOptions) => void }) {
  const [tab, setTab] = useState<G36DiagnosisPreset>("minimal");
  const [manualOptions, setManualOptions] = useState<G36DiagnosisOptions>(initialOptions);
  ...
  const activeOptions = useMemo(() => (tab === "manual" ? manualOptions : g36OptionsForPreset(tab)), [tab, manualOptions]);
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="g36-start-title" className="relative z-10 flex max-h-[90vh] w-full max-w-xl flex-col overflow-hidden rounded-xl border border-cyber-border bg-cyber-panel shadow-2xl">
        <div className="border-b border-cyber-border/40 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 id="g36-start-title" className="font-display text-base font-semibold text-white">3-6 백업·테스트 파일</h2>
              <p className="mt-1 text-xs text-cyber-muted">wordlist GET + 2-2 leak 판정 — httpx</p>
            </div>
            ...
            {TABS.map(({ id, icon: Icon, desc }) => {
              ...
              return (
                <button key={id} type="button" role="tab" aria-selected={active} onClick={() => setTab(id)} className={`rounded-md px-2 py-2.5 text-center transition ${active ? "bg-cyber-panel shadow-sm ring-1 ring-cyan-400/40" : "text-cyber-muted hover:text-white"}`}>
                  ...
                  <span className="block text-[11px] font-semibold text-white">{G36_PRESET_LABELS[id]}</span>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {tab === "minimal" ? <PresetOverview preset="minimal" options={MINIMAL_G36_OPTIONS} /> : null}
          {tab === "full" ? <PresetOverview preset="full" options={FULL_G36_OPTIONS} /> : null}
          {tab === "manual" ? <G36DiagnosisOptionsPanel options={manualOptions} onChange={setManualOptions} /> : null}
        </div>
        <div className="flex items-center justify-between gap-3 border-t border-cyber-border/40 bg-cyber-panel/80 px-5 py-3">
          <p className="hidden min-w-0 truncate font-mono text-[10px] text-cyan-300/70 sm:block">{g36OptionsSummary(activeOptions)}</p>
