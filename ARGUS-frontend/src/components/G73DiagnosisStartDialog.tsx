...
import { Eye, Gauge, Settings2, Stethoscope, X, Zap, ZapOff } from "lucide-react";
...
import {
  ...
  type G73DiagnosisPreset,
} from "../lib/g73DiagnosisOptions";

const TABS: {
  id: G73DiagnosisPreset;
  icon: typeof Gauge;
  desc: string;
}[] = [
  {
...
function PresetOverview({
  ...
  const checks = isMinimal
    ? [
        ...
        "httpx HEAD/GET — Server·X-Powered-By 등 헤더 점검",
        "ZAP 미사용 (수 초~1분)",
        "Strict ON · CDN 헤더 제외",
      ]
    : [
        "api-tree inventory 전체 경로 probe",
        "httpx — 버전·제품명 노출, 헤더 휴리스틱",
        "ZAP passive — Rule 10036 / 10037",
        "동일 헤더 이슈 finding 1건으로 collapse",
      ];
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-center gap-2">
        ...
        >
          {isMinimal ? <Gauge className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
        </span>
        <div>
          <p className="text-sm font-semibold text-white">{G73_PRESET_LABELS[preset]}</p>
          <p className="text-[10px] text-cyber-muted">
            {isMinimal ? "파이프라인·연결만 빠르게 확인" : "가능한 전 범위 헤더 노출 점검"}
          </p>
        </div>
      </div>
...
export function G73DiagnosisStartDialog({
  ...
  const [tab, setTab] = useState<G73DiagnosisPreset>("minimal");
  const [manualOptions, setManualOptions] = useState<G73DiagnosisOptions>(initialOptions);
  ...
  const activeOptions = useMemo(() => {
    ...
    return g73OptionsForPreset(tab);
  }, [tab, manualOptions]);
    ...
    >
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-cyber-muted">
                Server·X-Powered-By 등 응답 헤더의 정보 노출 여부 점검
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {tab === "minimal" ? (
            <PresetOverview preset="minimal" options={MINIMAL_G73_OPTIONS} />
          ) : null}
          {tab === "full" ? <PresetOverview preset="full" options={FULL_G73_OPTIONS} /> : null}
          {tab === "manual" ? (
            <G73DiagnosisOptionsPanel options={manualOptions} onChange={setManualOptions} />
          ) : null}
        </div>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-cyber-border/40 bg-cyber-panel/80 px-5 py-3">
          <p className="hidden min-w-0 truncate font-mono text-[10px] text-cyan-300/70 sm:block">
            {g73OptionsSummary(activeOptions)}
          </p>
