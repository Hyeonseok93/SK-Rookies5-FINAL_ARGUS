"""Orchestrate guideline 7-1 insecure HTTP method scan."""
...
def _collapse_method_findings(
    ...
    for f in items:
        ev = f.evidence or {}
        issue_type = ev.get("issue_type")
        if not issue_type:
            others.append(f)
            continue

        matched = tuple(ev.get("matched_methods") or ())
        key = (
            f"{f.severity}|{ev.get('base_url')}|{issue_type}|{ev.get('reason')}|"
            f"{','.join(matched)}|{ev.get('plugin_id', '')}"
        )
    ...
    for key, f in groups.items():
        ...
        if urls:
            ev["url"] = urls[0]
        issue_type = ev.get("issue_type", "insecure_method")
        if len(urls) > 1:
            sample = labels[0] if labels else urls[0]
            message = f"[7-1] {issue_type} — {len(urls)} URL(s), e.g. {sample}"
        else:
            message = f.message
        collapsed.append(
            DiagnosisFinding(
...
def run_g71_scan(ctx: DiagnosisContext, module_dir: Path) -> ScanResult:
    _ = module_dir
    opts = _scan_options(ctx.raw_config)

    targets_mod = _load_local("targets")
    probes_mod = _load_local("probes")
    rules_mod = _load_local("method_rules")
    ...
    prepare(len(probe_targets), f"7-1: {len(probe_targets)} URL(s)")

    scan_rules = rules_mod.scan_rules_from_config(ctx.raw_config)
    findings, stats = probes_mod.run_method_probes(
        probe_targets,
        classify_trace_fn=rules_mod.classify_trace_response,
        classify_allow_fn=rules_mod.classify_allow_header,
        strict_risky=scan_rules["strict_risky"],
    ...
    findings, collapse_stats = _collapse_method_findings(findings)
    ...
    if status == "pass" and stats.get("unreachable", 0) == probed:
        ...
        message = f"No insecure HTTP methods on {probed} probe(s) ({opts.probe_mode})"
    else:
        message = (
            f"Insecure HTTP methods: {collapsed} unique issue(s) from {probed} probe(s) "
            f"({opts.probe_mode})"
        )
