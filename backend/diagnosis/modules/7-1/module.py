"""Diagnosis module 7-1: Client Request Method."""
...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g71_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 7-1 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g71_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G71Module(DiagnosisModule):
    section_id = "7-1"
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g71_scan(ctx, self.module_dir)
