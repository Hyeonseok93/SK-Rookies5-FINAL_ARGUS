"""Orchestrate guideline 7-4 weak security configuration scan."""
...
def _collapse_security_findings(
    ...
    for f in items:
        ev = f.evidence or {}
        check_type = ev.get("check_type")
        if not check_type:
            others.append(f)
            continue

        key = (
            f"{f.severity}|{ev.get('base_url')}|{check_type}|{ev.get('header')}|"
            f"{ev.get('cookie_name')}|{ev.get('reason')}"
        )
    ...
    for key, f in groups.items():
        ...
        if len(urls) > 1:
            ...
            message = (
                f"[7-4] Weak security config ({ev.get('reason')}) — "
                f"{len(urls)} URL(s), e.g. {sample}"
            )
        else:
            label = labels[0] if labels else ev.get("url", "")
            message = f"[7-4] Weak security config ({ev.get('reason')}) on {label}"
        collapsed.append(
            DiagnosisFinding(
                severity=f.severity,
                message=message,
...
def run_g74_scan(ctx: DiagnosisContext, module_dir: Path) -> ScanResult:
    _ = module_dir
    opts = _scan_options(ctx.raw_config)

    targets_mod = _load_local("targets")
    probes_mod = _load_local("probes")
    rules_mod = _load_local("security_rules")
    ...
    for finding in findings:
        ...
        if ev.get("source") == "zap" or not ev.get("check_type"):
            continue
    ...
    if opts.zap_enabled:
        ...
        try:
            zap_phase("ZAP 7-4 security scan…")
    ...
    findings, collapse_stats = _collapse_security_findings(findings)
    ...
    if status == "pass" and stats.get("unreachable", 0) == probed:
        ...
        message = f"No weak security config on {probed} probe(s) ({opts.probe_mode})"
    else:
        message = (
            f"Weak security config: {collapsed} unique issue(s) from {probed} probe(s) "
            f"({opts.probe_mode})"
        )
