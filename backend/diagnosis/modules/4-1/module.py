"""Diagnosis module 4-1: 쿠키(Cookie) 및 웹 스토리지(Web Storage) 조작 가능성."""
...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g41_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 4-1 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g41_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G41Module(DiagnosisModule):
    section_id = "4-1"
    title = "쿠키(Cookie) 및 웹 스토리지(Web Storage) 조작 가능성"
    chapter = 4
    implemented = True
    engine = "httpx"
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g41_scan(ctx, self.module_dir)
