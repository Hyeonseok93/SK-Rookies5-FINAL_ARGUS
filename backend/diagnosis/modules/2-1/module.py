...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g21_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 2-1 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g21_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G21Module(DiagnosisModule):
    section_id = "2-1"
    title = "악성코드파일 업로드"
    chapter = 2
    implemented = True
