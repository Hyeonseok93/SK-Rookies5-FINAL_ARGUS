...
from g21_models import InputSource, ParamLocation, ScanParam, ScanTarget
...
def load_scan_targets(
    ...
    *,
    ...
    api_eps = [
        ...
        for ep in scoped
        if ep.kind == "api"
        and _is_api_base(ep.base_url)
        and (
            any(
                "multipart" in str(req.get("content_type", "")).lower()
                for req in getattr(ep, "requests", []) or []
            )
            or any("file" in p.name.lower() or "image" in p.name.lower() or "thumb" in p.name.lower() for p in ep.request_params or [])
            or ep.path.endswith("/upload")
            or "upload" in ep.path
        )
    ]
