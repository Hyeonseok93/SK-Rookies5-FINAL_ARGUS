"""Diagnosis module 1-6: input size and integrity validation."""
...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g16_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 1-6 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g16_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G16Module(DiagnosisModule):
    section_id = "1-6"
    title = "입력 값 크기 및 무결성 검증 오류"
    chapter = 1
    implemented = True
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g16_scan(ctx, self.module_dir)
