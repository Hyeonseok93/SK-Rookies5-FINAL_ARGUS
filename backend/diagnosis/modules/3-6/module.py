"""Diagnosis module 3-6: 백업 파일 및 테스트 파일 존재 여부."""
...
def _load_scanner():
    spec = importlib.util.spec_from_file_location("diag_g36_scanner", _MODULE_DIR / "scanner.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot load 3-6 scanner")
    mod = importlib.util.module_from_spec(spec)
    import sys

    sys.modules["diag_g36_scanner"] = mod
    spec.loader.exec_module(mod)
    return mod


class G36Module(DiagnosisModule):
    section_id = "3-6"
    title = "백업 파일 및 테스트 파일 존재 여부"
    chapter = 3
    implemented = True
    engine = "httpx"
    ...
    def run(self, ctx: DiagnosisContext) -> SectionReport:
        ...
        result = scanner.run_g36_scan(ctx, self.module_dir)
