...
def _scan_options(raw: dict[str, Any]) -> ScanOptions:
    ...
    return ScanOptions(
        ...
        idor_probe_enabled=bool(cfg.get("idor_probe_enabled", True)),
    )
...
def run_g22_scan(ctx: DiagnosisContext, module_dir: Path) -> ScanResult:
    ...
    design_findings = _design.review_design(candidates)
    stats: dict[str, Any] = {
    ...
    browse_paths = _payloads.load_forced_browse_paths(assets)
