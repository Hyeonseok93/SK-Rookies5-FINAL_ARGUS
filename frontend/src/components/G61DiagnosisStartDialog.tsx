...
export function G61DiagnosisStartDialog({
  ...
  const [options, setOptions] = useState<G61DiagnosisOptions>(G61_SMOKE_PRESET);
  const [activeTab, setActiveTab] = useState<G61OptionsTab>("smoke");

  useEffect(() => {
    if (open) {
      setOptions(initialOptions);
      setActiveTab(g61DetectTab(initialOptions));
    }
  }, [open, initialOptions]);
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-cyber-muted">
              오류페이지 정보 노출 — 탭으로 스모크 / 전체 전수 프리셋을 선택하세요.
            </p>
            <p className="mt-1 font-mono text-[10px] text-cyan-300/80">{g61OptionsSummary(options)}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          >
            ...
            {activeTab === "exhaustive" ? "전체 전수 스캔 시작" : "Start 6-1"}
          </button>
          <button
            type="button"
            onClick={onClose}
