...
export function G36DiagnosisStartDialog({
  open,
  initialOptions,
  onClose,
  onStart,
  ...
  open: boolean;
  initialOptions: G36DiagnosisOptions;
  onClose: () => void;
  onStart: (options: G36DiagnosisOptions) => void;
}) {
  const [options, setOptions] = useState<G36DiagnosisOptions>(DEFAULT_G36_OPTIONS);
  ...
  return (
    ...
    >
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="g36-start-title"
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 id="g36-start-title" className="font-display text-base font-semibold text-white">
              3-6 Diagnosis options
            </h2>
            <p className="mt-1 text-xs text-cyber-muted">
              백업·테스트 파일 — wordlist GET + 2-2 leak 판정 + optional ZAP 40035
            </p>
            <p className="mt-1 font-mono text-[10px] text-cyan-300/80">{g36OptionsSummary(options)}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <G36DiagnosisOptionsPanel options={options} onChange={setOptions} />
