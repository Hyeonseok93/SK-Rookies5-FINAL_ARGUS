...
export function G35DiagnosisStartDialog({
  open,
  initialOptions,
  onClose,
  onStart,
  ...
  open: boolean;
  initialOptions: G35DiagnosisOptions;
  onClose: () => void;
  onStart: (options: G35DiagnosisOptions) => void;
}) {
  const [options, setOptions] = useState<G35DiagnosisOptions>(DEFAULT_G35_OPTIONS);
  ...
  return (
    ...
    >
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="g35-start-title"
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 id="g35-start-title" className="font-display text-base font-semibold text-white">
              3-5 Diagnosis options
            </h2>
            <p className="mt-1 text-xs text-cyber-muted">
              검색엔진 노출 — robots.txt + noindex/nofollow 인벤토리 (httpx)
            </p>
            <p className="mt-1 font-mono text-[10px] text-cyan-300/80">{g35OptionsSummary(options)}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <G35DiagnosisOptionsPanel options={options} onChange={setOptions} />
