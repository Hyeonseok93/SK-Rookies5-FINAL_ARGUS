...
import { Gauge, Layers, Search, Settings2, Stethoscope, X, ZapOff } from "lucide-react";
import { G35DiagnosisOptionsPanel } from "./G35DiagnosisOptionsPanel";
import {
  FULL_G35_OPTIONS,
  G35_PRESET_LABELS,
  MINIMAL_G35_OPTIONS,
  ...
  type G35DiagnosisPreset,
} from "../lib/g35DiagnosisOptions";

const TABS: { id: G35DiagnosisPreset; icon: typeof Gauge; desc: string }[] = [
  { id: "minimal", icon: ZapOff, desc: "Base `/` + robots.txt" },
  { id: "full", icon: Layers, desc: "api-tree GET 전체" },
  { id: "manual", icon: Settings2, desc: "옵션 직접 설정" },
];

function PresetOverview({ preset, options }: { preset: "minimal" | "full"; options: G35DiagnosisOptions }) {
  const isMinimal = preset === "minimal";
  const checks = isMinimal
    ? ["Base `/` + robots.txt만", "noindex/nofollow 인벤토리", "anonymous + test-account pass", "수 초~1분"]
    : ["api-tree GET path 전수", "robots.txt + meta robots", "frontend base 우선 probe", "시간 크게 증가 가능"];
  return (
    <div className={`rounded-xl border p-4 ${isMinimal ? "border-cyan-400/30 bg-gradient-to-br from-cyan-500/10 to-transparent" : "border-amber-400/30 bg-gradient-to-br from-amber-500/10 to-transparent"}`}>
      <div className="mb-3 flex items-center gap-2">
        <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${isMinimal ? "bg-cyan-500/20 text-cyan-300" : "bg-amber-500/20 text-amber-200"}`}>
          {isMinimal ? <Gauge className="h-4 w-4" /> : <Search className="h-4 w-4" />}
        </span>
        <div>
          <p className="text-sm font-semibold text-white">{G35_PRESET_LABELS[preset]}</p>
          <p className="text-[10px] text-cyber-muted">{isMinimal ? "검색엔진 노출 빠른 확인" : "inventory 페이지 전수 noindex 점검"}</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-cyber-border/40 bg-cyber-bg/50 px-3 py-2">
        ...
        <p className="mt-1 font-mono text-[10px] leading-relaxed text-cyan-300/90">{g35OptionsSummary(options)}</p>
      </div>
    </div>
  );
}

export function G35DiagnosisStartDialog({ open, initialOptions, onClose, onStart }: { open: boolean; initialOptions: G35DiagnosisOptions; onClose: () => void; onStart: (options: G35DiagnosisOptions) => void }) {
  const [tab, setTab] = useState<G35DiagnosisPreset>("minimal");
  const [manualOptions, setManualOptions] = useState<G35DiagnosisOptions>(initialOptions);
  ...
  const activeOptions = useMemo(() => (tab === "manual" ? manualOptions : g35OptionsForPreset(tab)), [tab, manualOptions]);
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="g35-start-title" className="relative z-10 flex max-h-[90vh] w-full max-w-xl flex-col overflow-hidden rounded-xl border border-cyber-border bg-cyber-panel shadow-2xl">
        <div className="border-b border-cyber-border/40 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 id="g35-start-title" className="font-display text-base font-semibold text-white">3-5 검색엔진 노출</h2>
              <p className="mt-1 text-xs text-cyber-muted">robots.txt + noindex/nofollow — httpx inventory</p>
            </div>
            ...
            {TABS.map(({ id, icon: Icon, desc }) => {
              ...
              return (
                <button key={id} type="button" role="tab" aria-selected={active} onClick={() => setTab(id)} className={`rounded-md px-2 py-2.5 text-center transition ${active ? "bg-cyber-panel shadow-sm ring-1 ring-cyan-400/40" : "text-cyber-muted hover:text-white"}`}>
                  ...
                  <span className="block text-[11px] font-semibold text-white">{G35_PRESET_LABELS[id]}</span>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {tab === "minimal" ? <PresetOverview preset="minimal" options={MINIMAL_G35_OPTIONS} /> : null}
          {tab === "full" ? <PresetOverview preset="full" options={FULL_G35_OPTIONS} /> : null}
          {tab === "manual" ? <G35DiagnosisOptionsPanel options={manualOptions} onChange={setManualOptions} /> : null}
        </div>
        <div className="flex items-center justify-between gap-3 border-t border-cyber-border/40 bg-cyber-panel/80 px-5 py-3">
          <p className="hidden min-w-0 truncate font-mono text-[10px] text-cyan-300/70 sm:block">{g35OptionsSummary(activeOptions)}</p>
